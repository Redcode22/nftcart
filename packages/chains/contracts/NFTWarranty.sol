...
contract NFTWarranty is ERC721URIStorage, Ownable {
  ...
  function getListedByTokenID(uint256 tokenID) public view returns (PublishedToken memory) {
  ...
  function mint(string memory tokenURI, string memory serialID) public payable returns(uint256) {
    _tokenID.increment();
  ...
  function syncToBlockchain(uint256 tokenID,string memory serialID) private {
    ...
    function getPublishedByUser() public view returns (PublishedToken[] memory) {
        ...
        {
            if(publishedTokens[i+1].owner == msg.sender || publishedTokens[i+1].seller == msg.sender){
                itemCount += 1;
            }
        }
    ...
    function transferByID(uint256 tokenID , uint expiry) public payable {
    ...
    function burnNFTByID(uint256 tokenID) public payable {
        ...
        require(block.timestamp>publishedTokens[tokenID].expiry,"Warranty is expired");
